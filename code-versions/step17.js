...
function draw() {
    ...
    // ★ Hover efekti - taşı bırakmadan önce göster ★
    if (!gameOver && mouseY >= OFFSET_Y) {
        ...
        if (hoverCol >= 0 && hoverCol < COLS && gameBoard[0][hoverCol] == 0) {
            ...
            if (currentPlayer == 1) {
                fill(255, 0, 0, 150);  // Yarı saydam kırmızı
            } else {
                fill(255, 255, 0, 150);  // Yarı saydam sarı
            }
            noStroke();
            ellipse(x, 20, PIECE_SIZE * 0.8, PIECE_SIZE * 0.8);
        }
    }
    ...
    if (gameOver) {
        ...
        textSize(16);
        textAlign(LEFT);
        text((currentPlayer == 1) ? "Sıra: Kırmızı" : "Sıra: Sarı", 10, 25);
    }
}
